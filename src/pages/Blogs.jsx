...
// Added charm configurations for each article card
        ...
        <section className="mb-16">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {FEATURED_BLOGS.map((blog) => {
              const CharmIcon = blog.charm.icon;
              return (
                <div
                  key={blog.id}
                ...
                >
                  {/* --- RETRO CHARM PIN (Slides out from right edge on hover) --- */}
                  <div className="absolute top-10 -right-2 z-30 pointer-events-none translate-x-full opacity-0 group-hover:translate-x-2 group-hover:opacity-100 transition-all duration-300 ease-out">
                    <div
                      className={`px-2.5 py-1 border-2 border-black rounded-full ${blog.charm.bg} ${blog.charm.text} shadow-brutal text-[9px] font-black uppercase flex items-center gap-1.5 rotate-6 group-hover:rotate-3 transition-transform`}
                    >
                      <CharmIcon className="w-3 h-3" />
                      <span>{blog.charm.label}</span>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative h-44 border-b-3 border-black overflow-hidden rounded-t-[13px]">
                      ...
                      <div className="absolute top-2 left-2 flex gap-1.5">
                        <span
                          className={`text-[9px] font-black uppercase px-2 py-0.5 rounded border-2 border-black ${CATEGORY_COLORS[blog.category]}`}
                        >
                          {blog.category}
                        </span>
                      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 items-center gap-6">
            <div className="md:col-span-8">
              ...
              <p className="font-bold text-xs sm:text-sm uppercase text-black/80 max-w-xl">
                ...
                original perspectives. If you have an idea or draft — we'd love
                to publish it!
              </p>
            </div>
