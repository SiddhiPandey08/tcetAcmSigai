...
const FAQItem = ({ q, a }) => {
  ...
  return (
    <div className="border-3 border-black rounded-2xl overflow-hidden shadow-brutal-sm">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between p-4 bg-white hover:bg-retroYellow transition-colors font-black text-left gap-4"
      >
        <span>{q}</span>
        <span
          className={`text-xl transition-transform duration-200 flex-shrink-0 ${open ? "rotate-45" : ""}`}
        >
          +
        </span>
      </button>
      {open && (
        <div className="px-4 pb-4 bg-white border-t-2 border-black">
          <p className="text-gray-700 font-medium pt-3">{a}</p>
        </div>
      )}
    </div>
  );
  ...
  return (
    <div className="space-y-12 pb-16">
      ...
      <section className="bg-retroYellow border-3 border-black rounded-3xl p-8 md:p-12 shadow-brutal-lg text-center relative overflow-hidden">
        {/* Decorative dots */}
        <div className="absolute top-4 left-6 w-4 h-4 bg-black rounded-full opacity-20" />
        <div className="absolute bottom-6 right-10 w-6 h-6 bg-retroPink border-2 border-black rounded-full" />
        <div className="absolute top-8 right-8 w-3 h-3 bg-retroGreen border-2 border-black rounded-full" />

        <span className="bg-retroPink text-black font-black text-xs px-4 py-1.5 border-2 border-black rounded-full uppercase tracking-wider inline-block mb-4">
          ★ GET IN TOUCH
        </span>
      ...
      <section>
        <div className="text-center mb-8">
          <span className="bg-retroPink text-black font-black text-xs px-4 py-1.5 border-2 border-black rounded-full uppercase tracking-wider">
            FAQ
          </span>
          <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tight text-black mt-3">
            Quick FAQs
          </h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-4xl mx-auto">
          <FAQItem
            q="How do I join TCET ACM SIGAI?"
            a="Watch out for our recruitment drives at the start of each academic year, or reach out to us directly via Instagram or email. We're always looking for passionate students!"
          />
          <FAQItem
            q="Do I need prior AI/ML experience to join?"
            a="Not at all! We welcome students from all backgrounds. We have events and workshops for beginners right through to advanced learners."
          />
          <FAQItem
            q="Can I collaborate with SIGAI on a project?"
            a="Absolutely. We love cross-team and cross-college collaborations. Email us at acmtcet8@gmail.com with a brief about your idea."
          />
          <FAQItem
            q="Where is the AIML department located?"
            a="We're at TCET's 4th floor, AIML Department, Kandivali (E). Look for the ACM notice board near the department entrance!"
          />
        </div>
      </section>
